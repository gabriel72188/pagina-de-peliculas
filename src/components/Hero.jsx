...
const Hero = ({ movies }) => {
  ...
  return (
    <div className="relative w-full max-w-[100vw] h-[60vh] md:h-[85vh] bg-black group overflow-hidden">
      ...
      {/* ... (Texto y Botón) ... */}
      <div className="absolute top-0 left-0 w-full h-full flex flex-col justify-center px-8 md:px-16 z-20">
         <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold text-white drop-shadow-lg max-w-3xl leading-tight">{movie.title || movie.name}</h1>
         {/* ... metadata ... */}
         <div className="flex items-center gap-4 mt-4 text-sm md:text-base font-semibold text-gray-200">
         ...
         <p className="hidden md:block text-gray-300 mt-4 text-lg max-w-2xl line-clamp-3 drop-shadow leading-relaxed">{movie.overview}</p>

         <div className="flex gap-4 mt-8">
             {/* CAMBIO EN EL ONCLICK DEL BOTÓN */}
             <button 
                onClick={() => handlePlay(movie)} // Usamos la nueva función de navegar
                className="bg-white text-black text-lg font-bold py-3 px-8 rounded-md hover:bg-gray-200 transition flex items-center gap-2"
             >
                Reproducir
             </button>
         </div>
      ...
      <button onClick={prevSlide} className="hidden group-hover:block absolute left-2 md:left-8 top-1/2 -translate-y-1/2 bg-transparent border-none outline-none text-white hover:text-gray-200 transition-transform hover:scale-110 z-30 p-2">
      ...
      <button onClick={nextSlide} className="hidden group-hover:block absolute right-2 md:right-8 top-1/2 -translate-y-1/2 bg-transparent border-none outline-none text-white hover:text-gray-200 transition-transform hover:scale-110 z-30 p-2">
